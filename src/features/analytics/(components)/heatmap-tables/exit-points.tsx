...
export type Data = HeatmapWrapperProps['zonePathsTable']['exit_points'][number]

interface Props {
	className?: string
	data: Data[]
}
const ExitPointsTable = ({ className, data }: Props) => {
	...
	const exportExcel = useCallback(async () => {
		...
		try {
			await exportEntryOrExitData({
				data: data,
				title: t('Analytics.heatmap.exitDataExcelTitle'),
				filename: t('Analytics.heatmap.exitFilename', {
...
export default ExitPointsTable
