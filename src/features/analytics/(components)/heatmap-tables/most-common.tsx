...
import { exportMostCommonData } from '@/features/analytics/services/most-common-data-export.ts'

export type Data =
	HeatmapWrapperProps['zonePathsTable']['most_common_transitions'][number]

interface Props {
	className?: string
	data: Data[]
}
const MostCommonTable = ({ className, data }: Props) => {
			...
			columnHelper.accessor('transition', {
				header: ({ column }) => (
					<DataTableColumnHeader
						column={column}
						title='Analytics.heatmap.transition'
					/>
				),
	...
	const exportExcel = useCallback(async () => {
		setIsExporting(true)
		try {
			await exportMostCommonData({
				data: data,
				title: t('Analytics.heatmap.mostCommonExcelTitle'),
				filename: t('Analytics.heatmap.mostCommonFilename', {
