...
export function TemplateUpsertDialog({
	...
	const projectMutation = useMutation({
		...
		onSuccess: async (res) => {
			...
			toast.success(
				t(isEdit ? 'Templates.updateSuccess' : 'Templates.createSuccess', {
					value: res.name,
				})
			)
		...
		>
			<DialogContent className='sm:max-w-lg'>
				<DialogHeader className='text-left'>
					<DialogTitle>{t(`Templates.${isEdit ? 'edit' : 'add'}`)}</DialogTitle>
					<DialogDescription>
						{t(`Templates.${isEdit ? 'editDescription' : 'addDescription'}`)}
					</DialogDescription>
				</DialogHeader>
